...
The script:
1.  Flattens ``adata.obs["mapping_cell_type"]`` by stripping the trailing
    ``.N`` suffix (e.g. FOL.5 → FOL) to produce a broad group label that
    matches keys in ``GENE_GROUPS``.
2.  For each broad group that has ≥1 cell and a matching gene group,
    produces a PDF with one row per marker gene.
3.  Within each broad group, ``fine_clust`` provides the sub-cluster labels
    (e.g. IFE.Keratinocytes, IFE.Basal) used for x-axis grouping and the
    colour bar.
"""
...
# ── gene groups ──────────────────────────────────────────────────────────────
# Keys match the broad group derived from mapping_cell_type by stripping .N.
#
# NOTE: FIB is included as a placeholder — populate with marker genes as
# needed.  The script will skip any group whose gene list is empty or whose
# genes are absent from the data.

GENE_GROUPS: dict[str, list[str]] = {
    "FOL": [
        "KRT15", "NOTCH2", "COL11A1",
        "KRT6B", "DKK3",
        "COMP",
        "KRT85", "LGR5", "KRT19",
        "KRT75",
    ...
    "EPI": [
        "MOXD1", "KRT1", "KRT10", "KRT6B", "SLC15A1",
        "NDRG1", "IFITM3", "COL17A1", "LAMB4",
        "NABP1", "S100A7",
    ],
    "ENDO": [
        "GJA4", "SELE", "ACKR1", "PECAM1", "VWF",
        "RGS5", "NOTCH3", "SEMA3G"
    ],
    "IMM": [
        "SKAP1", "CD69", "CD68", "C1QA", "CD207", "CD1A",
        "KLRB1", "IL26", "FOXP3", "CTLA4", "LYZ",
    ],
    "NC": [
        "EDNRB", "MITF", "S100B", "ZEB1", "CRABP1", "OCA2", "RBMS3", "PMEL", "MLANA"
    ],
    "FIB": [
        "RGS5", "PTGDS", "ADAM12",
        "TAGLN", "CNN1", "DKK2", "VCAN",
        "FGF7",
    ],
}
...
def plot_genes(
    ...
    cell_groups : Series     (index = cell barcodes, values = fine_clust labels),
                  already sorted by cluster.
    genes       : list of gene names to plot (rows).
    cmap        : dict mapping fine_clust label → colour.
...
def plot_umap(
    ...
    """
    UMAP scatter: cells belonging to *cell_type* (in the *obs_col* column)
    are highlighted in a single colour; all other cells are drawn in silver
    behind them.

    Parameters
    ----------
    adata            : full AnnData object (all cells).
    cell_type        : the broad group label to highlight.
    outpath          : output PDF path.
    obs_col          : obs column to match *cell_type* against.
    ...
    fg_mask = adata.obs[obs_col] == cell_type
...
def main():
    ...
    # ── derive broad_cell_type by flattening mapping_cell_type ──
    if "mapping_cell_type" not in adata.obs.columns:
        sys.exit("ERROR: adata.obs must contain a 'mapping_cell_type' column.")
    ...
    # FOL.5 → FOL, EPI.3 → EPI, ENDO.1 → ENDO, etc.
    adata.obs["broad_cell_type"] = (
    ...
    # ── iterate over broad cell types ──
    broad_types = sorted(adata.obs["broad_cell_type"].unique())
    print(f"Found {len(broad_types)} broad cell types: {broad_types}")

    for broad_type in broad_types:

        # ── pick the matching gene group ──
        if broad_type not in GENE_GROUPS:
            print(f"\nSkipping '{broad_type}' — no gene group defined.")
            continue

        genes = [g for g in GENE_GROUPS[broad_type] if g in dataset_full.index]
        if not genes:
            print(f"\nSkipping '{broad_type}' — none of its marker genes are in the data.")
            continue

        mask = adata.obs["broad_cell_type"] == broad_type
        barcodes = adata.obs_names[mask]
        if len(barcodes) == 0:
            continue

        print(f"\nProcessing '{broad_type}' ({len(barcodes)} cells) ...")
        ...
        # output path
        outdir = os.path.join(args.outdir, broad_type.lower())
        os.makedirs(outdir, exist_ok=True)
        figname = f"{broad_type.lower()}_marker_genes.pdf"
        outpath = os.path.join(outdir, figname)

        plot_genes(dataset_sub, cell_groups, genes, cmap, outpath)

        # ── UMAP highlight plot ──
        umap_figname = f"{broad_type.lower()}_umap.pdf"
        umap_outpath = os.path.join(outdir, umap_figname)
        plot_umap(adata, broad_type, umap_outpath, obs_col="broad_cell_type")
