...
obj <- readRDS(paste0(main_folder, "annotated.RDS"))
...
for (cond in conditions) {
  ...
  # Subset to condition
  obj_subset <- subset(obj, subset = dataset == cond)
...
gg1 <- compareInteractions(cellchat, show.legend = FALSE, group = c(1, 2, 3))
gg2 <- compareInteractions(cellchat, show.legend = FALSE, group = c(1, 2, 3), measure = "weight")
...
gg1 <- rankNet(cellchat, mode = "comparison", stacked = TRUE, do.stat = TRUE, comparison = c(1,2,3))
gg2 <- rankNet(cellchat, mode = "comparison", stacked = FALSE, do.stat = TRUE, comparison = c(1,2,3))
...
################################################
# 8. IDENTIFY SPECIFIC L-R PAIR CHANGES IN BULGE
################################################
